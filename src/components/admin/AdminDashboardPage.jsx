...
export default function AdminDashboardPage({
  ...
  // useEffect(() => {
  //   if (liveAdminData?.email_address) {
  //     refreshAdminDetails();
  //   }
  // }, [liveAdminData?.email_address]);
  ...
  const [liveAdminData, setLiveAdminData] = useState({});
  const getLiveAdminDetails = async () => {
    ...
    try {
      ...
      const adminEndpoint =
        ...
        import.meta.env.VITE_READ_ADMIN_BY_ID +
        "?id=" +
        adminDetails.id;
      // alert(adminEndpoint);
      const adminResponse = await axiosAdminInstance.get(
        adminEndpoint,
        // requestData,
        {
          headers: {
      ...
      setLiveAdminData(adminResponse.data.data); // Update state with  count

      // openNotificationModal(true, currentPageName, "");
      // alert(JSON.stringify(adminResponse.data.data), null, 2); // Update state with payments count
  ...
  const getMessages = async () => {
    ...
    try {
      ...
      const messagesResponse = await axiosAdminInstance.post(
        messagesEndpoint,
        requestData,
        {
          headers: {
  ...
  useEffect(() => {
    if (liveAdminData) {
      setFormData({
        first_name: liveAdminData.firstname || "",
        middle_name: liveAdminData.middlename || "",
        last_name: liveAdminData.lastname || "",

        gender: liveAdminData.gender || "",
        phone_number: liveAdminData.phone_number || "",
        address: liveAdminData.address || "",
        date_of_birth: liveAdminData.date_of_birth || "",
        profile_image: liveAdminData.profile_image || "",

        // zone_id: liveAdminData.zone_id || "",
        // state_id: liveAdminData.state_id || "",
        // lga_id: liveAdminData.lga_id || "",
        // ward_id: liveAdminData.ward_id || "",

        // status: liveAdminData.status || "",
        // political_party: liveAdminData.political_party || "",
        // occupation: liveAdminData.occupation || "",
        // education_level: liveAdminData.education_level || "",
        // admin_type: liveAdminData.admin_type || "",
      });
    }
  }, [liveAdminData]);
    ...
    <div>
      ...
      <div className="flex">
        ...
        >
          ...
          {/* {liveAdminData &&
            liveAdminData.payment_receipt_status !== "Ok" &&
            // liveAdminData.payment_receipt == "" && 
            (
              <PaymentReceiptUpload
                liveAdminData={liveAdminData}
                refreshAdminDetails={refreshAdminDetails}
              />
            )} */}
                        ...
                        >
                          ...
                          {/* <div className="flex gap-5 mb-2">
                            ...
                            <p className="text-gray-600 text-sm">
                              State:{" "}
                              <span className="font-bold">
                                {liveAdminData?.state_name}
                              </span>
                            </p>
                            <p className="text-gray-600 text-sm">
                              LGA:{" "}
                              <span className="font-bold">
                                {liveAdminData?.lga_name}
                              </span>
                            </p>
                            <p className="text-gray-600 text-sm">
                              Ward:{" "}
                              <span className="font-bold">
                                {liveAdminData?.ward_name}
                              </span>
                            </p>
                          </div> */}
                        </motion.div>
                    ...
                    <>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                        ...
                        <div className="flex items-center bg-white shadow-md p-4 rounded-xl">
                          ...
                          <div className="ml-3">
                            <p className="text-gray-600 text-sm">DGF Admin ID:</p>
                            <p className="text-xl font-bold">
                              {liveAdminData?.tmf_id}
                            </p>
                          </div>
                        </div>
                        ...
                        >
                          ...
                          <div className="ml-3">
                            ...
                            {/* <p className="text-xl font-bold">{liveAdminData?.profile_percent ?? "82%"}</p> */}
                          </div>
                        </div>
