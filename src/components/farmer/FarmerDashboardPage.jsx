...
import FarmerSideNavbar from "../../navbar/farmer/FarmerSideNavbar.jsx";
...
export default function FarmerDashboardPage({
  ...
  // useEffect(() => {
  //   if (liveFarmerData?.email_address) {
  //     refreshFarmerDetails();
  //   }
  // }, [liveFarmerData?.email_address]);
  ...
  useEffect(() => {
    // alert(farmerDetails?.id);
    if (farmerDetails?.id) {
      getLiveFarmerDetails();
      // getMessages();
    }
  }, [farmerDetails?.id]);

  const [liveFarmerData, setLiveFarmerData] = useState({});
  const getLiveFarmerDetails = async () => {
    ...
    try {
      ...
      const farmerEndpoint =
        (import.meta.env.VITE_IS_LIVE === "true"
          ? import.meta.env.VITE_API_SERVER_URL
          : import.meta.env.VITE_API_DEMO_SERVER_URL) +
        import.meta.env.VITE_READ_FARMER_BY_ID +
        "?id=" +
        farmerDetails.id;
      // alert(farmerEndpoint);
      const farmerResponse = await axiosInstance.get(
        farmerEndpoint,
        // requestData,
        {
          headers: {
      ...
      setLiveFarmerData(farmerResponse.data.data); // Update state with  count

      // openNotificationModal(true, currentPageName, "");
      // alert(JSON.stringify(farmerResponse.data.data), null, 2); // Update state with payments count
  ...
  useEffect(() => {
    if (liveFarmerData) {
      setFormData({
        first_name: liveFarmerData.firstname || "",
        middle_name: liveFarmerData.middlename || "",
        last_name: liveFarmerData.lastname || "",

        gender: liveFarmerData.gender || "",
        phone_number: liveFarmerData.phone_number || "",
        address: liveFarmerData.address || "",
        date_of_birth: liveFarmerData.date_of_birth || "",
        profile_image: liveFarmerData.profile_image || "",

        // zone_id: liveFarmerData.zone_id || "",
        // state_id: liveFarmerData.state_id || "",
        // lga_id: liveFarmerData.lga_id || "",
        // ward_id: liveFarmerData.ward_id || "",

        // status: liveFarmerData.status || "",
        // political_party: liveFarmerData.political_party || "",
        // occupation: liveFarmerData.occupation || "",
        // education_level: liveFarmerData.education_level || "",
        // membership_type: liveFarmerData.membership_type || "",
      });
    }
  }, [liveFarmerData]);
    ...
    <div>
      ...
      <div className="flex">
        {isMobile ? (
          ...
          <FarmerSideNavbar currentPageName={currentPageName} />
        )}
        ...
        >
          ...
          {/* {liveFarmerData &&
            liveFarmerData.payment_receipt_status !== "Ok" &&
            // liveFarmerData.payment_receipt == "" && 
            (
              <PaymentReceiptUpload
                liveFarmerData={liveFarmerData}
                refreshFarmerDetails={refreshFarmerDetails}
              />
            )} */}
                        ...
                        >
                          ...
                          {/* <div className="flex gap-5 mb-2">
                            ...
                            <p className="text-gray-600 text-sm">
                              State:{" "}
                              <span className="font-bold">
                                {liveFarmerData?.state_name}
                              </span>
                            </p>
                            <p className="text-gray-600 text-sm">
                              LGA:{" "}
                              <span className="font-bold">
                                {liveFarmerData?.lga_name}
                              </span>
                            </p>
                            <p className="text-gray-600 text-sm">
                              Ward:{" "}
                              <span className="font-bold">
                                {liveFarmerData?.ward_name}
                              </span>
                            </p>
                          </div> */}
                        </motion.div>
                    ...
                    <>
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 w-full">
                        ...
                        <div className="flex items-center bg-white shadow-md p-4 rounded-xl">
                          ...
                          <div className="ml-3">
                            <p className="text-gray-600 text-sm">DGF Farmer ID:</p>
                            <p className="text-xl font-bold">
                              {liveFarmerData?.tmf_id}
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center bg-white shadow-md p-4 rounded-xl">
                          <div
                            className={`p-3 rounded-lg ${
                              liveFarmerData?.status === "Verified"
                                ? "bg-green-500/20"
                                : "bg-yellow-500/20"
                            }`}
                          >
                        ...
                        >
                          ...
                          <div className="ml-3">
                            ...
                            {/* <p className="text-xl font-bold">{liveFarmerData?.profile_percent ?? "82%"}</p> */}
                          </div>
                        </div>
                      </div>
                    </>
